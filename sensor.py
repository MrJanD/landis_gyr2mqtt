...
class HeatMeterSensor(CoordinatorEntity, RestoreSensor):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if self.key == "heat_usage":
            self._attr_native_value = self.coordinator.data.heat_usage_mwh or convert_gj_to_mwh(
                self.coordinator.data.heat_usage_gj
            )

        if self.key == "heat_previous_year":
            self._attr_native_value = self.coordinator.data.heat_previous_year_mwh or convert_gj_to_mwh(
